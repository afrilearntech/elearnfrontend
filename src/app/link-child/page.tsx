...
export default function LinkChildPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        const parentUrl = process.env.NEXT_PUBLIC_PARENT_TEACHER_URL || 'http://localhost:3003';
        if (typeof window !== 'undefined' && token) {
          const userStr = localStorage.getItem('user');
          const userData = userStr ? encodeURIComponent(userStr) : '';
          window.location.href = `${parentUrl}/dashboard?token=${encodeURIComponent(token)}${userData ? `&user=${userData}` : ''}`;
        }
      }, 1500);
    } catch (error: unknown) {
